package com.zsgame.zonesshift.environments.mapmanagment;

import android.content.Context;

import com.zsgame.zonesshift.environments.Tile;
import com.zsgame.zonesshift.gamestates.createlvl.mapeditor.TileSimplified;
...
public class MapLoader {
    ...
    public static TileSimplified[][] loadSimplifiedMapFromString(String mapData) {
        String[] lines = mapData.split("\n");
        int rows = lines.length;
        int cols = 0;
        ...
        TileSimplified[][] tiles = new TileSimplified[rows][cols];
        ...
        for (int row = 0; row < rows; row++) {
            ...
            for (int col = 0; col < line.length(); col++) {
                char c = line.charAt(col);
                if (c != '.') {
                    tiles[row][col] = new TileSimplified(c);
                }
            }
        }
